...
A single-purpose macOS utility: drop in a vertical marketing video, type a short block of Instagram Story-style text, position it by dragging, and export a new MP4 with the text permanently burned into the pixels. Everything runs locally—no cloud, no accounts, no analytics.
...
2. Type into Story Text—the overlay updates live, preserving your line breaks and wrapping long lines automatically.
3. Adjust font, size, alignment, color, background, and padding in the right-hand panel.
4. Drag the text block anywhere on the preview, or use the Top, Center, and Bottom quick-position buttons.
...
1. **Core Graphics renders the text block once**—font, color, alignment, rounded background box, and drop shadow (when there is no box)—at full source-video resolution ([OverlayRenderer.swift](Sources/StoryStamper/Rendering/OverlayRenderer.swift)).
2. **The preview displays that same bitmap**, scaled into the aspect-fit video rect. The block's position is stored in normalized (0...1) video coordinates, so resizing the window never moves the exported text.
3. **On export**, the block is composited onto a transparent canvas at full video size, saved as a temporary PNG, and FFmpeg overlays it at (0, 0) for the whole duration ([VideoExporter.swift](Sources/StoryStamper/Export/VideoExporter.swift)).
...
- One text block, shown for the entire video—no animation, trimming, filters, or multiple overlays, by design.
