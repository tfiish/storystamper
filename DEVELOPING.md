...
- `KeyEquivalent.delete` is backspace, and only a menu shortcut ever matches it. A key *press* reports the Delete key as DEL, so `onKeyPress` and `keyboardShortcut` need different constants for the same physical key—`VideoPreviewView.deleteKey` is the one for presses. Matching the wrong one fails silently: the handler simply never runs.
